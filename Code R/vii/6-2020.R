...
thang11 <- mydata2[mydata2$date >= "2020-11-1" & mydata2$date <= "2020-11-30", ]
thang12 <- mydata2[mydata2$date >= "2020-12-1" & mydata2$date <= "2020-12-31", ]

thang11$date <- day(thang11$date)
thang12$date <- day(thang12$date)

ketqua <- ggplot()+
  geom_line(data = thang11, mapping = aes(x = date, y = new_deaths, color = "November"))+
  geom_line(data = thang12, mapping = aes(x = date, y = new_deaths, color = "December"))+
  scale_color_manual(name = "Note:", values = c("November" = "orange", "December" = "green"))
require(scales)
res <- ketqua + scale_y_continuous(labels = comma)
print(res + ggtitle("New deaths in Nov-Dec of 2020")
